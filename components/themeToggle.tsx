...
// Source for below animation:
// https://codepen.io/lerida/pen/ZEbWBjp

const ThemeToggle = ({ size }: { size: number }) => {
  const { colorMode, toggleColorMode } = useColorMode();

  let moon = useRef<SVGGElement | null>(null);
  let sun = useRef<SVGGElement | null>(null);
  let stars = useRef<SVGGElement | null>(null);
  let cloud = useRef<SVGPathElement | null>(null);

  useEffect(() => {
    gsap.set([sun.current, cloud.current, moon.current], { x: 15 });
    gsap.set([sun.current, cloud.current], { opacity: 0 });
    gsap.set([stars.current, moon.current], { opacity: 1 });
    gsap.set(moon.current, { x: -157 });
    gsap.set(stars.current, { x: 35, y: -5 });

    // gsap.set([moon.current, stars.current], { opacity: 0 });
    // gsap.set(stars.current, { x: 35, y: -5 });
    // gsap.set([sun.current, cloud.current, moon.current], { x: 15 });
  }, []);
  ...
  const nightFn = () => {
    ...
    gsap.to(cloud.current, { opacity: 1, ease: Power1.easeInOut, duration: 1 });
    gsap.to(moon.current, {
      opacity: 0,
      x: 35,
      rotate: 360,
    ...
    gsap.to(stars.current, { opacity: 0, ease: Power1.easeInOut, duration: 1 });
  };
  ...
  return (
    <button onClick={toggleFn} style={{ outline: "none" }}>
      <svg
        version="1.1"
        ...
        style={{
          ...
          height: "auto",
          backgroundColor: colorMode === "dark" ? "lightblue" : "yellow",
          borderRadius: "50px",
        }}
      >
        <g ref={stars}>
